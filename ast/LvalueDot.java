   ...
   implements Lvalue
{
   ...
   public Type typeCheck(Map<String, IdProperties> symTable,
      ...
      if (!(leftType instanceof StructType)) {
         System.out.println(this.lineNum + ": Accessing field from a non-struct type");
         System.exit(-1);
      }
      String leftName = ((StructType)leftType).getName();
      if (!(structTable.containsKey(leftName))) {
         System.out.println(this.lineNum + ": There is no struct named '" + leftName + "'");
         System.exit(-1);
      }
      if (!((structTable.get(leftName)).containsKey(id))) {
         System.out.println(this.lineNum + ": id '"+id+"' is not a field of struct '"+leftName+"'");
         System.exit(-1);
      }
